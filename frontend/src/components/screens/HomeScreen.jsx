...
const HomeScreen = ({ onNavigate }) => {
  return (
    <div className="h-full bg-gradient-to-b from-emerald-50 to-white">
      {/* Header */}
      <div className="px-4 pt-2 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1">
            <span className="text-lg font-semibold text-gray-800">Niro.AI</span>
          </div>
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-full px-4 py-3 flex items-center gap-3 shadow-lg shadow-emerald-200"
        >
          <MessageSquare className="w-5 h-5 text-white" />
          <span className="text-white text-sm flex-1 text-left">Ask Niro.AI anything...</span>
          <div className="flex items-center gap-1">
      ...
      <div className="px-4 mb-4">
        <div className="flex gap-2">
          ...
          <button className="px-4 py-1.5 text-xs bg-emerald-500 text-white rounded-full font-medium">Today</button>
        ...
        <div className="flex flex-wrap gap-2">
          {quickQuestions.slice(0, 3).map((q, i) => (
            <button
              key={i}
              onClick={() => onNavigate('chat')}
              className="text-xs bg-white border border-gray-200 rounded-full px-3 py-1.5 text-gray-600 hover:bg-emerald-50 hover:border-emerald-200 transition-colors"
            >
              {q}
            </button>
          ))}
